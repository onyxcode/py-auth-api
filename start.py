...
def open_account(username, slt, hash):
    db = mongo.web_users
    db[f"{username}"].insert_one({"_id": "credentials", "salt": slt, "hashed": hash})
...
def change_password(username, request, new_password):
    ...
    if request.json["username"] in col_list:
        ...
        if bcrypt.checkpw(passwd, user_doc["hashed"]):
            ...
            db[f"{username}"].update_one(
                {"_id": "credentials"}, {"$set": {"salt": salt, "hashed": hashed}}, True
            )
...
async def post_handler(request):
    if (
        ...
        and request.json["new_password"]
    ):
        change = change_password(
            request.json["username"], request, request.json["new_password"]
        )
